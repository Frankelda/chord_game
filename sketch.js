...
function createCheckBoxes() {
  ...
  currentY += space;
  sharpNoteCheckBox = createCheckbox(" Sharps", true);
  sharpNoteCheckBox.position(xPos, currentY);
  currentY += space;
  flatNoteCheckBox = createCheckbox(" Flats", true);
  ...
  currentY += space;
  minorCheckBox = createCheckbox(" Minor", true);
  ...
  for (let i = 0; i < boxes.length; i++) {
    ...
    currentBox.style("font-size", (18).toString() + "px");
...
function createStartStopButtons() {
  let buttonWidthPercentOfHalf = 0.5;
  let buttonHeightPercent = 0.15;
  let percentOfButtonHeightGap = 0.3;

  let boarderThickness = width / 200;
  ...
  startButton.style("background-color", color('#31e00d'));
  ...
  startButton.mouseOver(() => startButton.style("background-color", adjust('#31e00d', 50))).mouseOut(() => {
    startButton.style("background-color", color('#31e00d'));
  ...
  stopButton.mouseOver(() => stopButton.style("background-color", adjust(BACKGROUND, 0))).mouseOut(() => {
    stopButton.style("background-color", color(BACKGROUND));
...
function start() {
  ...
  stopButton.style("background-color", color( '#f0140c'));
  stopButton.style("color", color(255));
   stopButton.mouseOver(() => stopButton.style("background-color", adjust('#f0140c', 50))).mouseOut(() => {
    stopButton.style("background-color", color('#f0140c'));
    stopButton.style("color", color(255));
  });
  ...
  startButton.style("color", color(0));
   startButton.mouseOver(() => startButton.style("background-color", adjust(BACKGROUND, 0))).mouseOut(() => {
